...
`content/docs/index.mdx` — Task 6에서 옮긴 "8회차 한눈에 보기" 표를 지우고:
...
describe('<WhyStudy />', () => {
  ...
  it('미작성 칸을 작성 예정으로 표시한다', () => {
    render(<WhyStudy />)
    const drafts = why.filter((c) => c.draft)
    expect(screen.getAllByText('작성 예정')).toHaveLength(drafts.length)
  })
...
describe('<Roadmap />', () => {
  ...
  it('미작성 마일스톤을 작성 예정으로 표시한다', () => {
    render(<Roadmap />)
    expect(screen.getAllByText('작성 예정')).toHaveLength(roadmap.filter((m) => m.draft).length)
  })
})
```
 ...
 *
 * ── 채우는 법 ──
 * [1] 겪은 문제   — 이미 채워져 있다 (README 1장)
 * [2] 진단        — 왜 기존 스터디로는 이 문제가 안 풀리는지. 본인 경험으로.
 *                   body를 쓰고 draft를 지운다.
 * [3] 설계 결정   — 그 진단 때문에 이 스터디에 넣은 장치들.
 *                   links로 해당 시각화 페이지를 가리키면 이 페이지가 허브가 된다.
 */
...
스터디 내용은 전부 사이트에 있습니다 → **https://<배포주소>**
